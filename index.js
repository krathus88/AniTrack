    ...
    try 
    {
        ...
        else 
        {
            for (let i = 0; i < animeID.length; i++) 
            {
                ...
                const truncatedSynopsis = animeSearchResult.data.data.synopsis.substring(0, 400);
...
app.get("/search", async (req, res) => 
{
    try 
    {
        ...
        const animeSearchResult = await axios.get(API_URL + `/anime?q=${userInput}&page=${page}`);
...
app.get("/anime/:anime_title", (req, res) => 
{
    try 
    {
        activePage = "Anime";
...
app.post("/anime", async (req, res) => 
{
    try 
    {
        ...
        res.redirect(303, `/anime/${animeTitle}`); // 303 ensures redirect is a GET request
    } 
    catch (error) 
    {
        ...
        res.status(500).send("Internal Server Error");
        await delay(3500); // In case being rate limited, wait 3.5s and then redirect to main page
        res.redirect(303, "/"); // 303 ensures redirect is a GET request
    }
});
